...
void do_iteration(inout float iter, inout bool z_real_sign, inout uint[n_ints] z_real_num, inout bool z_imag_sign,  inout uint[n_ints] z_imag_num,
    ...
    for (int i=0; i<num_iter; i++) {
        ...
        if (float(z_real_sq_num[0]) + float(z_real_sq_num[1]) * 0.00001525878 + float(z_imag_sq_num[0]) + float(z_imag_sq_num[1]) * 0.00001525878 > 4) {
            break;
        }
...
void main() {
    ...
    if (pixel_coords.x < image_size.x && pixel_coords.y < image_size.y) {
        ...
        bool c_real_sign = c_buf.table[coord * (n_ints + 1) * 2] == 1;
        bool c_imag_sign = c_buf.table[coord * (n_ints + 1) * 2 + (n_ints + 1)] == 1;

        bool z_real_sign = z_buf.table[coord * (n_ints + 1) * 2] == 1;
        bool z_imag_sign = z_buf.table[coord * (n_ints + 1) * 2 + (n_ints + 1)] == 1;

        for (int i=0; i<n_ints; i++) {
            c_real_num[i] = c_buf.table[coord * (n_ints + 1) * 2 + i + 1];
            c_imag_num[i] = c_buf.table[coord * (n_ints + 1) * 2 + (n_ints + 1) + i + 1];
